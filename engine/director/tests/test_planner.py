"""LLM 개입 선택의 순수 로직 — 하드룰 클램핑이 핵심 (ADR-013).

LLM은 행동 반경을 넓힐 수 없다: 화이트리스트 밖 사건은 거부(폴백), 환각 액터는
...
def test_plan_schema_kinds_are_enum():
...
def test_valid_plan_keeps_llm_framing_and_records_selector():
    plan = {
    ...
    result = intervention_from_plan(plan, SNAP, TENSION, INCIDENTS, NAMES, model="qwen2.5:14b")
    assert result is not None
    assert result.tool == "inject_incident"
    assert result.incident_kind == "rumor_spread"
    assert result.description == plan["description"]  # LLM의 맥락 서술 보존
    assert result.intensity == 0.7
    assert result.affected_actor_ids == ["a_minji"]
    assert result.location_id == "loc_office"  # 라이브러리에서만 — LLM이 못 지어낸다
    assert result.reason == plan["rationale"]
    assert result.signals["selector"] == "llm"
    assert result.signals["model"] == "qwen2.5:14b"


def test_unknown_kind_returns_none_for_fallback():
    plan = {
        "incident_kind": "earthquake",  # 라이브러리 밖 — 존재하지 않는 도구
        "affected_actor_ids": ["a_minji"],
        "description": "지진",
        "intensity": 0.9,
        "rationale": "x",
...
def test_hallucinated_actors_dropped_falls_back_to_top_pair():
    ...
    assert result is not None
    assert result.affected_actor_ids == ["a_minji", "a_seongho"]  # 상위 긴장 쌍으로 폴백
...
def test_partial_hallucination_keeps_only_valid_candidates():
    ...
    result = intervention_from_plan(plan, SNAP, TENSION, INCIDENTS, NAMES)
    assert result is not None
    assert result.affected_actor_ids == ["a_minji"]  # 환각만 제거, 유효는 유지
...
def test_intensity_is_clamped_and_defaults_to_library():
    ...
    def intensity_of(plan: dict) -> float:
        return intervention_from_plan(plan, SNAP, TENSION, INCIDENTS, NAMES).intensity
...
def test_empty_description_falls_back_to_library():
    ...
    result = intervention_from_plan(plan, SNAP, TENSION, INCIDENTS, NAMES)
    assert result.description == "카페 정전"  # 라이브러리 서술로 폴백
...
def test_no_tension_yields_pure_environmental_incident():
    ...
    assert result is not None
    assert result.affected_actor_ids == []  # 영향권 없는 순수 환경 사건도 유효


def test_build_plan_user_grounds_names_kinds_and_candidates():
    ...
    assert "a_minji" in user  # 후보 id 명시 (영향권 화이트리스트)
